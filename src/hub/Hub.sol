...
contract Hub is Multicall, Auth, Recoverable, IHub, IHubRequestManagerCallback, IHubGuardianActions {
    ...
    function callRequestManager(PoolId poolId, uint16 centrifugeId, bytes calldata data) external payable {
        ...
        (bool success, bytes memory returnData) =
            address(hubRegistry.hubRequestManager(poolId, centrifugeId)).call{value: msg.value}(data);
        if (!success) {
    ...
    function _isManager(PoolId poolId) internal view {
        ...
        address caller = address(batcher) != address(0) ? batcher : msg.sender;
        require(hubRegistry.manager(poolId, caller), IHub.NotManager());
    }
}
