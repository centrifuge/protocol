...
interface INftEscrow {
    ...
    /// No more tokens from the same `tokenId` can be sent to the escrow until it's unlocked.
    ...
    /// @param source The contract or collection where identifies the `tokenId`.
    /// @param tokenId The asset identification.
