...
contract FlashLoanHelper is IFlashLoanHelper, IAaveV3FlashLoanReceiver {
        ...
        returns (bool)
    {
        ...
        (bytes32[] memory commands, bytes[] memory state, uint128 stateBitmap, uint8 fixedSlots) =
            abi.decode(params, (bytes32[], bytes[], uint128, uint8));
        IExecutor(executor).executeCallback(commands, state, stateBitmap, fixedSlots);
