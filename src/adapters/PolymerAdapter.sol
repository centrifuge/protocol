...
///
///         Polymer has no on-chain fee mechanism. Relaying proofs and paying for
...
contract PolymerAdapter is Auth, IPolymerAdapter {
    ...
    function receiveMessage(bytes calldata proof) external {
        ...
        // Topics layout: [eventSelector, centrifugeId, adapter, nonce] - each 32 bytes
        ...
        require(eventSelector == SEND_MESSAGE_SELECTOR, InvalidProof());

        // Validate the source adapter is wired and matches the emitting contract
        PolymerSource memory source = sources[chainId];
        require(source.addr != address(0) && source.addr == emittingContract, InvalidSource());

        // Validate the message is addressed to this adapter on this chain
        require(destAdapter == address(this), InvalidProof());

        // Replay protection: mark nonce as processed
        require(!processedNonces[chainId][sourceNonce], AlreadyProcessed());
        processedNonces[chainId][sourceNonce] = true;

        bytes memory payload = abi.decode(unindexedData, (bytes));
        ...
        returns (bytes32 adapterData)
    {
        ...
        emit SendMessage(centrifugeId, destination.addr, currentNonce, payload);
        adapterData = bytes32(currentNonce);
    }

    /// @inheritdoc IAdapter
    /// @dev Polymer sending is event-based with no on-chain fee. Returns 0.
