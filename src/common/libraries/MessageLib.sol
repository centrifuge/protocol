...
library MessageLib {
    ...
    /// and reduce generated bytecode
    // forgefmt: disable-next-item
    uint256 constant UPDATE_RESTRICTION_LENGTHS =
        (41 << uint8(UpdateRestrictionType.Member) * 8) +
        (33 << uint8(UpdateRestrictionType.Freeze) * 8) +
        (33 << uint8(UpdateRestrictionType.Unfreeze) * 8);
    ...
    function messageLength(bytes memory message) internal pure returns (uint16 length) {
        ...
        if (kind == uint8(MessageType.UpdateRestriction)) {
            uint8 restrictionKind = message.toUint8(length);
            require(restrictionKind <= uint8(type(UpdateRestrictionType).max), UnknownMessageType());

            length += uint16(uint8(bytes32(UPDATE_RESTRICTION_LENGTHS)[31 - restrictionKind]));
        }
    }
