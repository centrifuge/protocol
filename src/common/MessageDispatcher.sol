...
contract MessageDispatcher is Auth, IMessageDispatcher {
    ...
    /// @inheritdoc IPoolMessageSender
    function sendIssuedShares(PoolId poolId, ShareClassId scId, uint128 shareAmount, D18 pricePoolPerShare)
    external
    auth
    {
        if (assetId.centrifugeId() == localCentrifugeId) {
            investmentManager.issuedShares(poolId, scId, assetId, shareAmount, pricePoolPerShare);
        } else {
            gateway.send(
                assetId.centrifugeId(),
    ...
    /// @inheritdoc IPoolMessageSender
    function sendRevokedShares(PoolId poolId, ShareClassId scId, AssetId assetId, uint128 assetAmount, uint128 shareAmount, D18 pricePoolPerShare) external auth {
        if (assetId.centrifugeId() == localCentrifugeId) {
