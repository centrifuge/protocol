...
import {IVaultsRouter} from "src/vaults/interfaces/IVaultsRouter.sol";
...
///         It also adds the need to fully pay for each step of the transaction execution. VaultsRouter allows
...
///         VaultsRouter. Any funds that do remain are at risk of being taken by other users.
contract VaultsRouter is Auth, Multicall, IVaultsRouter {
    ...
    function requestDeposit(address vault, uint256 amount, address controller, address owner)
        ...
        protected
    {
        require(owner == msg.sender || owner == address(this), "VaultsRouter/invalid-owner");
    ...
    function unlockDepositRequest(address vault, address receiver) external payable protected {
        ...
        require(lockedRequest != 0, "VaultsRouter/no-locked-balance");
    ...
    function executeLockedDepositRequest(address vault, address controller) external payable protected {
        ...
        require(lockedRequest != 0, "VaultsRouter/no-locked-request");
    ...
    function wrap(address wrapper, uint256 amount, address receiver, address owner) public payable protected {
        ...
        require(amount != 0, "VaultsRouter/zero-balance");
        SafeTransferLib.safeTransferFrom(underlying, owner, address(this), amount);

        _approveMax(underlying, wrapper);
        require(IERC20Wrapper(wrapper).depositFor(receiver, amount), "VaultsRouter/wrap-failed");
    }
    ...
    function unwrap(address wrapper, uint256 amount, address receiver) public payable protected {
        ...
        require(IERC20Wrapper(wrapper).withdrawTo(receiver, amount), "VaultsRouter/unwrap-failed");
    }
