...
import { AssetId, AsyncVault, BaseTest, ERC20, IShareToken, MessageLib, MockAdapter, PoolId, ShareClassId, VaultKind } from "../../core/spoke/integration/BaseTest.sol";
...
contract DepositTest is BaseTest {
    ...
    function _testDepositMint(uint256 amount, bool snap) internal {
        ...
        centrifugeChain.isFulfilledDepositRequest(
            vault.poolId().raw(), vault.scId().raw(), bytes32(bytes20(self)), assetId, uint128(amount), shares, 0
        );
        if (snap) {
            vm.stopSnapshotGas();
        }
    ...
    function testDepositAsEndorsedOperator(uint256 amount) public {
        ...
        centrifugeChain.isFulfilledDepositRequest(
            vault.poolId().raw(), vault.scId().raw(), bytes32(bytes20(self)), assetId, uint128(amount), sharePayout, 0
        );
    ...
    function testDepositAndRedeemPrecisionWithInverseDecimals(bytes16 scId) public {
        ...
        centrifugeChain.isFulfilledDepositRequest(
            poolId, scId, bytes32(bytes20(self)), assetId, assets, firstSharePayout, 0
        );
        ...
        centrifugeChain.isFulfilledDepositRequest(
            poolId, scId, bytes32(bytes20(self)), assetId, assets, secondSharePayout, 0
        );
    ...
    function testDepositAndRedeemPrecisionWithSlippage(bytes16 scId) public {
        ...
        centrifugeChain.isFulfilledDepositRequest(poolId, scId, bytes32(bytes20(self)), assetId, assets, shares, 0);
