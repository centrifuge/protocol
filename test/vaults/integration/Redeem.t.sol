...
contract RedeemTest is BaseTest {
    ...
    function testRedeem(uint256 amount) public {
        ...
        vault.requestRedeem(amount, address(this), address(this));
        assertEq(shareToken.balanceOf(address(poolEscrowFactory.escrow(vault.poolId().raw()))), amount);
        ...
        assertEq(shareToken.balanceOf(address(poolEscrowFactory.escrow(vault.poolId().raw()))), 0);
        assertEq(erc20.balanceOf(address(poolEscrowFactory.escrow(vault.poolId().raw()))), assets);
        ...
        assertTrue(shareToken.balanceOf(address(poolEscrowFactory.escrow(vault.poolId().raw()))) <= 1);
        assertTrue(erc20.balanceOf(address(poolEscrowFactory.escrow(vault.poolId().raw()))) <= 1);
    ...
    function testTriggerRedeemRequestTokens(uint128 amount) public {
        ...
        assertApproxEqAbs(shareToken.balanceOf(address(poolEscrowFactory.escrow(vault.poolId().raw()))), amount / 2, 1);
        ...
        asyncRequests.triggerRedeemRequest(poolId, scId, investor, AssetId.wrap(assetId), uint128(amount + 1));

        //Fail - Share token amount zero
        vm.expectRevert(IAsyncRequests.ShareTokenAmountIsZero.selector);
        asyncRequests.triggerRedeemRequest(poolId, scId, investor, AssetId.wrap(assetId), 0);

        // should work even if investor is frozen
        centrifugeChain.freeze(poolId.raw(), scId.raw(), investor); // freeze investor
        assertTrue(
            !ShareToken(address(vault.share())).checkTransferRestriction(
                investor, address(poolEscrowFactory.escrow(vault.poolId().raw())), amount
            )
        );

        // half of the amount will be trabsferred from the investor's wallet & half of the amount will be taken from
        // escrow
        centrifugeChain.triggerIncreaseRedeemOrder(poolId.raw(), scId.raw(), investor, assetId, amount);

        assertApproxEqAbs(shareToken.balanceOf(investor), 0, 1);
        assertApproxEqAbs(shareToken.balanceOf(address(poolEscrowFactory.escrow(vault.poolId().raw()))), amount, 1);
    ...
    function testTriggerRedeemRequestTokensWithCancellation(uint128 amount) public {
        ...
        assertEq(shareToken.balanceOf(address(poolEscrowFactory.escrow(vault.poolId().raw()))), amount / 2);
        ...
        centrifugeChain.triggerIncreaseRedeemOrder(poolId.raw(), scId.raw(), investor, assetId, amount / 2);
        assertApproxEqAbs(shareToken.balanceOf(investor), 0, 1);
        assertApproxEqAbs(shareToken.balanceOf(address(poolEscrowFactory.escrow(vault.poolId().raw()))), amount, 1);
        assertEq(vault.maxMint(investor), 0);
    }
    ...
    function testTriggerRedeemRequestTokensUnmintedTokensInEscrow(uint128 amount) public {
        ...
        centrifugeChain.triggerIncreaseRedeemOrder(poolId.raw(), scId.raw(), investor, assetId, redeemAmount);
        assertApproxEqAbs(shareToken.balanceOf(address(poolEscrowFactory.escrow(vault.poolId().raw()))), amount, 1);
    ...
    function testPartialRedemptionExecutions() public {
        ...
        asset.mint(address(poolEscrowFactory.escrow(vault.poolId().raw())), assets * 2 - investmentAmount);
