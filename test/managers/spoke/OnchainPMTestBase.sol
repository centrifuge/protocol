...
abstract contract OnchainPMTestBase is Test {
    using CastLib for *;
    ...
    IOnchainPM.Callback[] internal NO_CALLBACKS;

    function _callback(bytes32 h, address caller) internal pure returns (IOnchainPM.Callback[] memory arr) {
        arr = new IOnchainPM.Callback[](1);
        arr[0] = IOnchainPM.Callback(h, caller);
    }

    function _callbacks(bytes32 h0, address c0, bytes32 h1, address c1)
        internal
        pure
        returns (IOnchainPM.Callback[] memory arr)
    {
        arr = new IOnchainPM.Callback[](2);
        arr[0] = IOnchainPM.Callback(h0, c0);
        arr[1] = IOnchainPM.Callback(h1, c1);
    }
    ...
    function _computeScriptHash(bytes32[] memory commands, bytes[] memory state, uint128 stateBitmap)
        ...
        returns (bytes32)
    {
        return _computeScriptHash(commands, state, stateBitmap, new IOnchainPM.Callback[](0));
    }
