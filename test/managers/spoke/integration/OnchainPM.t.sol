...
contract OnchainPMMulticallTest is OnchainPMTestBase {
        ...
        returns (bytes memory)
    {
        ...
        return
            abi.encodeWithSelector(IOnchainPM.execute.selector, commands, state, bitmap, NO_CALLBACKS, new bytes32[](0));
    }
}

// ─── Multicall executes batched scripts ──────────────────────────────────────

contract OnchainPMMulticallBatchTest is OnchainPMMulticallTest {
    function testMulticallExecutesTwoScripts() public {
        ...
        calls[0] = abi.encodeWithSelector(IOnchainPM.execute.selector, cmdsA, stateA, bitmapA, NO_CALLBACKS, proofA);
        calls[1] = abi.encodeWithSelector(IOnchainPM.execute.selector, cmdsB, stateB, bitmapB, NO_CALLBACKS, proofB);
...
// ─── OnchainPM → SlippageGuard integration ──────────────────────────────────
...
// ─── OnchainPM → FlashLoanHelper → executeCallback integration ────────────
...
contract OnchainPMFlashLoanTest is OnchainPMTestBase {
    ...
    function testFlashLoanCallback() public {
        ...
        // Encode callback data for Aave pool → FlashLoanHelper.executeOperation → OnchainPM.executeCallback
        ...
        IOnchainPM.Callback[] memory callbacks = _callback(innerHash, address(flashReceiver));
        ...
        // OnchainPM's tokens reduced by fee amount
