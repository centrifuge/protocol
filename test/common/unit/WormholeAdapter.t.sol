...
contract WormholeAdapterSetRemoteAdapterTest is WormholeAdapterTestBase {
    ...
    function testSetRemoteAdapter() public {
        vm.expectEmit();
        emit IWormholeAdapter.File(
            "sources", REMOTE_CENTRIFUGE_CHAIN_ID, REMOTE_WORMHOLE_CHAIN_ID, REMOTE_ADAPTER_ADDRESS
        );
        vm.expectEmit();
        emit IWormholeAdapter.File(
            "destinations", REMOTE_CENTRIFUGE_CHAIN_ID, REMOTE_WORMHOLE_CHAIN_ID, REMOTE_ADAPTER_ADDRESS
        );

        adapter.setRemoteAdapter(REMOTE_CENTRIFUGE_CHAIN_ID, REMOTE_WORMHOLE_CHAIN_ID, REMOTE_ADAPTER_ADDRESS);

        (uint16 sourceCentrifugeId, address sourceAddr) = adapter.sources(REMOTE_WORMHOLE_CHAIN_ID);
        assertEq(sourceCentrifugeId, REMOTE_CENTRIFUGE_CHAIN_ID);
        assertEq(sourceAddr, REMOTE_ADAPTER_ADDRESS);

        (uint16 destWormholeId, address destAddr) = adapter.destinations(REMOTE_CENTRIFUGE_CHAIN_ID);
        assertEq(destWormholeId, REMOTE_WORMHOLE_CHAIN_ID);
        assertEq(destAddr, REMOTE_ADAPTER_ADDRESS);
    }

    function testSetRemoteAdapterRevertIfSourceAlreadyConfigured() public {
        adapter.file("sources", REMOTE_CENTRIFUGE_CHAIN_ID, REMOTE_WORMHOLE_CHAIN_ID, REMOTE_ADAPTER_ADDRESS);

        vm.expectRevert(abi.encodeWithSelector(IWormholeAdapter.SourceExists.selector));
        adapter.setRemoteAdapter(REMOTE_CENTRIFUGE_CHAIN_ID, REMOTE_WORMHOLE_CHAIN_ID, REMOTE_ADAPTER_ADDRESS);
    }

    function testSetRemoteAdapterRevertIfDestinationAlreadyConfigured() public {
        adapter.file("destinations", REMOTE_CENTRIFUGE_CHAIN_ID, REMOTE_WORMHOLE_CHAIN_ID, REMOTE_ADAPTER_ADDRESS);

        vm.expectRevert(abi.encodeWithSelector(IWormholeAdapter.DestinationExists.selector));
        adapter.setRemoteAdapter(REMOTE_CENTRIFUGE_CHAIN_ID, REMOTE_WORMHOLE_CHAIN_ID, REMOTE_ADAPTER_ADDRESS);
    }
    ...
    function testSetRemoteAdapterWithDifferentChainIds() public {
        uint16 otherCentrifugeId = 5;
        uint16 otherWormholeId = 6;

        adapter.setRemoteAdapter(REMOTE_CENTRIFUGE_CHAIN_ID, REMOTE_WORMHOLE_CHAIN_ID, REMOTE_ADAPTER_ADDRESS);

        // Should be able to set up another remote adapter with different IDs
        adapter.setRemoteAdapter(otherCentrifugeId, otherWormholeId, REMOTE_ADAPTER_ADDRESS);

        // Verify both configurations exist
        (uint16 sourceCentrifugeId1, address sourceAddr1) = adapter.sources(REMOTE_WORMHOLE_CHAIN_ID);
        assertEq(sourceCentrifugeId1, REMOTE_CENTRIFUGE_CHAIN_ID);
        assertEq(sourceAddr1, REMOTE_ADAPTER_ADDRESS);

        (uint16 sourceCentrifugeId2, address sourceAddr2) = adapter.sources(otherWormholeId);
        assertEq(sourceCentrifugeId2, otherCentrifugeId);
        assertEq(sourceAddr2, REMOTE_ADAPTER_ADDRESS);
    }

    function testSetRemoteAdapterFuzz(uint16 centrifugeId, uint16 wormholeId, address adapterAddr) public {
        vm.assume(centrifugeId != 0);
        vm.assume(wormholeId != 0);
        vm.assume(adapterAddr != address(0));

        adapter.setRemoteAdapter(centrifugeId, wormholeId, adapterAddr);

        (uint16 sourceCentrifugeId, address sourceAddr) = adapter.sources(wormholeId);
        assertEq(sourceCentrifugeId, centrifugeId);
        assertEq(sourceAddr, adapterAddr);

        (uint16 destWormholeId, address destAddr) = adapter.destinations(centrifugeId);
        assertEq(destWormholeId, wormholeId);
        assertEq(destAddr, adapterAddr);
    }
}
