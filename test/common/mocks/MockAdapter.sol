...
contract MockAdapter is Auth, Mock, IAdapter {
    ...
    function send(uint16, bytes calldata message, uint256, uint256, address)
        public
        payable
    ...
    function estimate(uint16, bytes calldata, uint256 gasLimit, uint256) public view returns (uint256 estimation) {
        estimation = values_uint256_return["estimate"] + gasLimit;
    }
}
