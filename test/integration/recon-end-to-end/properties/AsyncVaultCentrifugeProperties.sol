    ...
    AsyncVaultProperties
{
        ...
        returns (uint256 shares) {
            ...
            if (withdrawAmount == maxWithdrawBefore) {
                ...
                eq(
                    maxWithdrawAfter,
                    0,
                    "maxWithdrawAfter should be 0 if withdrawAmount == maxWithdrawBefore"
                );
                lte(
                    assets,
                    maxWithdrawBefore,
        ...
        try _getVault().redeem(redeemAmount, _getActor(), _getActor()) returns (
            ...
            if (redeemAmount == maxRedeemBefore) {
                ...
                eq(
                    maxRedeemAfter,
                    0,
                    "maxRedeemAfter should be 0 if redeemAmount == maxRedeemBefore"
                );
                eq(
                    pendingRedeem,
                    pendingRedeemBefore,
    ...
    function _validateAsyncMaxValueChange(
        ...
        uint256 expectedMaxValueAfter = maxValueBefore > operationAmount
        ...
        lte(
            ...
            string.concat(
                "Async ",
                operationName,
                ": maxValue should decrease by approximately operationAmount"
            )
        );
        gte(
            maxValueAfter,
            ...
            string.concat(
                "Async ",
                operationName,
                ": maxValue should not decrease by more than operationAmount"
            )
        );
    }
    ...
    function _validateSyncMaxValueChange(
        ...
        t(
            ...
            string.concat(
                operationName,
                ": total should increase by asset amount"
            )
        );
        ...
        if (state.isNormalStateBefore && state.isNormalStateAfter) {
            ...
            uint256 availableBalanceChange = state.availableBalanceAfter -
            ...
            if (keccak256(bytes(operationName)) == keccak256(bytes("Mint"))) {
                ...
                expectedAmount = _getVault().convertToShares(
            ...
            bool withinBounds = actualDecrease >= lowerBound &&
                actualDecrease <= upperBound;
            bool isConversionCapped = actualDecrease == 0 &&
                maxValueBefore == maxValueAfter;
    ...
    function _captureAsyncClaimStateBefore(
        IBaseVault vault,
        address receiver
    ) internal view returns (AsyncClaimState memory state) {
        address shareToken = vault.share();
        address globalEscrowAddr = address(asyncRequestManager.globalEscrow());

        state.globalEscrowSharesBefore = IERC20(shareToken).balanceOf(
            globalEscrowAddr
        );
        state.receiverSharesBefore = IERC20(shareToken).balanceOf(receiver);

        (state.maxMintBefore, , , , , , , , , ) = asyncRequestManager
            .investments(vault, _getActor());
        return state;
    }

    /// @dev Updates async claim state after vault.deposit() operation
    /// @notice Updates the state struct with post-operation values
    function _updateAsyncClaimStateAfter(
        AsyncClaimState memory state,
        IBaseVault vault,
        address receiver
    ) internal view {
        address shareToken = vault.share();
        ...
        state.globalEscrowSharesAfter = IERC20(shareToken).balanceOf(
            globalEscrowAddr
        );
        state.receiverSharesAfter = IERC20(shareToken).balanceOf(receiver);

        (state.maxMintAfter, , , , , , , , , ) = asyncRequestManager
            .investments(vault, _getActor());
    }
    ...
    function _validateAsyncVaultClaim(
        AsyncClaimState memory state,
        uint256 depositAmount,
        string memory operationName
    ) internal {
        if (depositAmount == 0) {
            eq(
                ...
                string.concat(
                    operationName,
                    ": zero deposit should return zero shares"
                )
            );
            eq(
                ...
                string.concat(
                    operationName,
                    ": zero deposit should not change globalEscrow"
                )
            );
            eq(
                ...
                string.concat(
                    operationName,
                    ": zero deposit should not change receiver balance"
                )
            );
            eq(
                state.maxMintBefore,
                ...
                string.concat(
                    operationName,
                    ": zero deposit should not change maxMint"
                )
            );
            return;
        }

        uint256 globalEscrowDecrease = state.globalEscrowSharesBefore -
            state.globalEscrowSharesAfter;
        uint256 receiverIncrease = state.receiverSharesAfter -
            state.receiverSharesBefore;
        eq(
            ...
            string.concat(
                operationName,
                ": globalEscrow must decrease by exact shares returned"
            )
        );
        eq(
            receiverIncrease,
            ...
            string.concat(
                operationName,
                ": receiver must receive exact shares returned"
            )
        );
        eq(
            globalEscrowDecrease,
            ...
            string.concat(
                operationName,
                ": shares leaving globalEscrow must equal shares received"
            )
        );
        ...
        gte(
            ...
            string.concat(
                operationName,
                ": maxMint must decrease by at least shares returned"
            )
        );
        lte(
            maxMintDecrease,
            ...
            string.concat(
                operationName,
                ": maxMint must decrease by at at most shares returned +1 due to rounding"
            )
        );
    }
    ...
    function _validateAsyncDepositFailure(uint256 depositAmount) internal {
        ...
        if (!depositPrice.isZero()) {
            VaultDetails memory vaultDetails = vaultRegistry.vaultDetails(
                _getVault()
            );
