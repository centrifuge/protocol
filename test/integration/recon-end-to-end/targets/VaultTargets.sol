...
abstract contract VaultTargets is BaseTargetFunctions, Properties {
        ...
        updateGhostsWithType(OpType.REQUEST_DEPOSIT)
    {
        ...
        vm.prank(_getActor());
        try IAsyncVault(address(_getVault())).requestDeposit(assets, to, _getActor()) {
            // ghost tracking
            userRequestDeposited[_getVault().scId()][vaultRegistry.vaultDetails(_getVault()).assetId][to] += assets;
            ...
            // If not member
            (bool isMemberTo,) = fullRestrictions.isMember(_getVault().share(), to);
            if (!isMemberTo) {
    ...
    function vault_deposit(uint256 assets) public updateGhostsWithType(OpType.ADD) {
        ...
        uint256 shareUserB4;
        uint256 shareEscrowB4;
        uint256 shares;
        {
            bytes32 actorBytes = actor.toBytes32();
            PoolId poolId = vault.poolId();
            ShareClassId scId = vault.scId();
            AssetId assetId = vaultRegistry.vaultDetails(vault).assetId;

            shareUserB4 = IShareToken(shareToken).balanceOf(actor);
            shareEscrowB4 = IShareToken(shareToken).balanceOf(address(globalEscrow));
            (uint128 pendingBefore,) = batchRequestManager.depositRequest(poolId, scId, assetId, actorBytes);

            vm.prank(actor);
            shares = vault.deposit(assets, actor);

            (uint128 pendingAfter,) = batchRequestManager.depositRequest(poolId, scId, assetId, actorBytes);

            // for sync vaults, deposits are fulfilled and claimed immediately
            if (!isAsyncVault) {
                if (pendingBefore >= pendingAfter) {
                    sumOfFulfilledDeposits[shareToken] += shares;
                    sumOfClaimedDeposits[shareToken] += (pendingBefore - pendingAfter);
                }
                executedInvestments[shareToken] += shares;
                sumOfSyncDepositsAsset[vault.asset()] += assets;
                sumOfSyncDepositsShare[shareToken] += shares;
                userDepositProcessed[scId][assetId][actor] += assets;
                userRequestDeposited[scId][assetId][actor] += assets;
            }
        }

        // Bal after
        uint256 shareUserAfter = IShareToken(shareToken).balanceOf(actor);
        uint256 shareEscrowAfter = IShareToken(shareToken).balanceOf(address(globalEscrow));
    ...
    function vault_mint(uint256 shares) public updateGhostsWithType(OpType.ADD) {
        ...
        (uint128 pendingBefore,) = batchRequestManager.depositRequest(poolId, scId, assetId, toBytes);

        // NOTE: external calls above so need to prank directly here
        vm.prank(to);
        uint256 assets = vault.mint(shares, to);

        (uint128 pendingAfter,) = batchRequestManager.depositRequest(poolId, scId, assetId, toBytes);
        ...
        if (!isAsyncVault) {
            ...
            userDepositProcessed[scId][assetId][to] += assets;
            sumOfSyncDepositsAsset[vault.asset()] += assets;
            sumOfSyncDepositsShare[shareToken] += shares;
    ...
    function vault_redeem(uint256 shares, uint256 toEntropy) public updateGhostsWithType(OpType.REMOVE) {
        address to = _getRandomActor(toEntropy);
        address escrow = address(poolEscrowFactory.escrow(_getVault().poolId()));
