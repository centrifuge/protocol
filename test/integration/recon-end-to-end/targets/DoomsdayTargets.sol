...
abstract contract DoomsdayTargets is BaseTargetFunctions, Properties {
    ...
    function doomsday_zeroPrice_noPanics() public statelessTest {
        ...
        uint32 nowIssueEpoch = batchRequestManager.nowIssueEpoch(
            poolId,
            scId,
            assetId
        );
        ...
        {
            (uint128 approvedPool, , , , , ) = batchRequestManager
                .epochInvestAmounts(poolId, scId, assetId, nowIssueEpoch);
            eq(
                approvedPool,
                0,
                "approved pool amount should return 0 at zero price"
            );
        ...
        uint32 nowRevokeEpoch = batchRequestManager.nowRevokeEpoch(
            poolId,
            scId,
            assetId
        );
        ...
        {
            (, , , , uint128 payoutAssetAmount, ) = batchRequestManager
                .epochRedeemAmounts(poolId, scId, assetId, nowRevokeEpoch);
            eq(
                payoutAssetAmount,
                0,
            ...
            t(
                expectedRevert,
                "revokeShares shout not revert at zero price apart from EpochNotFound"
            );
        }
    }
}
