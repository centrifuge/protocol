...
contract BytesLibTest is Test {
    function testSymbolicSlice(bytes memory data, bytes memory randomStart, bytes memory randomEnd) public pure {
    ...
    function testSymbolicToAddress(address addr, bytes memory randomStart, bytes memory randomEnd) public pure {
    ...
    function testSymbolicToUint8(uint8 number, bytes memory randomStart, bytes memory randomEnd) public pure {
    ...
    function testSymbolicToUint16(uint16 number, bytes memory randomStart, bytes memory randomEnd) public pure {
        bytes memory value = bytes.concat(bytes.concat(randomStart, abi.encodePacked(number)), randomEnd);
        assertEq(BytesLib.toUint16(value, randomStart.length), number);
    }

    function testSymbolicToUint32(uint32 number, bytes memory randomStart, bytes memory randomEnd) public pure {
        bytes memory value = bytes.concat(bytes.concat(randomStart, abi.encodePacked(number)), randomEnd);
        assertEq(BytesLib.toUint32(value, randomStart.length), number);
    }

    function testSymbolicToUint64(uint64 number, bytes memory randomStart, bytes memory randomEnd) public pure {
        bytes memory value = bytes.concat(bytes.concat(randomStart, abi.encodePacked(number)), randomEnd);
        assertEq(BytesLib.toUint64(value, randomStart.length), number);
    }

    function testSymbolicToUint128(uint128 number, bytes memory randomStart, bytes memory randomEnd) public pure {
        bytes memory value = bytes.concat(bytes.concat(randomStart, abi.encodePacked(number)), randomEnd);
        assertEq(BytesLib.toUint128(value, randomStart.length), number);
    }

    function testSymbolicToBytes32(bytes32 data, bytes memory randomStart, bytes memory randomEnd) public pure {
        bytes memory value = bytes.concat(bytes.concat(randomStart, abi.encodePacked(data)), randomEnd);
        assertEq(BytesLib.toBytes32(value, randomStart.length), data);
    }

    function testSymbolicToBytes16(bytes16 data, bytes memory randomStart, bytes memory randomEnd) public pure {
        bytes memory value = bytes.concat(bytes.concat(randomStart, abi.encodePacked(data)), randomEnd);
