...
abstract contract Properties is BeforeAfter, Asserts, ERC7540CentrifugeProperties {
    ...
    function invariant_sentinel_tranche_balance() public trancheTokenIsSet {
    ...
    function invariant_global_1() public trancheTokenIsSet {
        // Mint and Deposit
        lte(
            sumOfClaimedDeposits[address(trancheToken)],
            sumOfFullfilledDeposits[address(trancheToken)],
            "sumOfClaimedDeposits not <= sumOfFullfilledDeposits"
        );
    }

    // Sum of underlying assets received on `redeem` and `withdraw` <= sum of underlying assets received on
    // `fulfillRedeemRequest`
    function invariant_global_2() public trancheTokenIsSet {
        // Redeem and Withdraw
        // investmentManager_handleExecutedCollectRedeem
        lte(
            sumOfClaimedRedemptions[address(token)],
            mintedByCurrencyPayout[address(token)],
            "sumOfClaimedRedemptions !<= mintedByCurrencyPayout"
        );
    }

    function invariant_global_2_inductive() public trancheTokenIsSet {
    ...
    function invariant_global_3() public trancheTokenIsSet {
    ...
    function invariant_global_4() public trancheTokenIsSet {
    ...
    function invariant_global_5() public trancheTokenIsSet {
        // claimCancelDepositRequest
        // investmentManager_fulfillCancelDepositRequest
        lte(
            sumOfClaimedDepositCancelations[address(token)],
    ...
    function invariant_global_5_inductive() public trancheTokenIsSet {
    ...
    function invariant_global_6() public trancheTokenIsSet {
        // claimCancelRedeemRequest
        lte(
            sumOfClaimedRedeemCancelations[address(trancheToken)],
    ...
    function invariant_global_6_inductive() public trancheTokenIsSet {
    ...
    function invariant_tt_2() public trancheTokenIsSet {
     ...
     */
    function invariant_E_1() public trancheTokenIsSet {
        ...
        uint256 balOfEscrow = token.balanceOf(address(escrow)) - tokenBalanceOfEscrowAtFork; // The balance of tokens in
     ...
     *     NOTE: Ignores donations
     */
    function invariant_E_2() public trancheTokenIsSet {
        // NOTE: By removing checked the math can overflow, then underflow back, resulting in correct calculations
        // NOTE: Overflow should always result back to a rational value as trancheToken cannot overflow due to other
        // functions permanently reverting
