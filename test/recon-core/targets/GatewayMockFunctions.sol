...
import {UpdateRestrictionType} from "src/common/libraries/MessageLib.sol";
...
abstract contract GatewayMockFunctions is BaseTargetFunctions, Properties {
        ...
        returns (address newToken, address newTranche, address newVault)
    {
...

        {
            ...
            poolManager_addTranche(POOL_ID, TRANCHE_ID, name, symbol, 18, bytes32(0), address(restrictionManager));
        }
    ...
    // Step 2
    function poolManager_addAsset(uint128 currencyId, address currencyAddress) public notGovFuzzing {
    ...
    // Step 5
    function poolManager_allowAsset(uint64 poolId, uint128 currencyId) public notGovFuzzing {
        poolManager.allowAsset(poolId, currencyId);
    }

    // Step 3
    function poolManager_addPool(uint64 poolId) public notGovFuzzing {
    ...
    function poolManager_addTranche(
        ...
        bytes32 salt,
        address hook
    ) public notGovFuzzing {
        poolManager.addTranche(poolId, trancheId, tokenName, tokenSymbol, decimals, salt, address(restrictionManager));
    }

    // Step 9
    function poolManager_deployTranche(uint64 poolId, bytes16 trancheId) public notGovFuzzing returns (address) {
        return poolManager.deployTranche(poolId, trancheId);
    }

    // Step 10
    function poolManager_deployVault(uint64 poolId, bytes16 trancheId, address currency)
        public
        notGovFuzzing
        returns (address)
    {
     ...
     */
    function poolManager_updateMember(uint64 validUntil) public notGovFuzzing {
        poolManager.updateRestriction(
            poolId,
            trancheId,
            abi.encodePacked(uint8(UpdateRestrictionType.Member), _getActor().toBytes32(), validUntil)
        );
    }

    // TODO: Price is capped at u64 to test overflows
    function poolManager_updateTranchePrice(uint64 price, uint64 computedAt) public notGovFuzzing {
        poolManager.updateTranchePrice(poolId, trancheId, currencyId, price, computedAt);
    }

    function poolManager_updateTrancheMetadata(string memory tokenName, string memory tokenSymbol)
        public
        notGovFuzzing
    {
    ...
    function poolManager_freeze() public notGovFuzzing {
        poolManager.updateRestriction(
            poolId, trancheId, abi.encodePacked(uint8(UpdateRestrictionType.Freeze), _getActor().toBytes32())
        );
    }

    function poolManager_unfreeze() public notGovFuzzing {
        poolManager.updateRestriction(
            poolId, trancheId, abi.encodePacked(uint8(UpdateRestrictionType.Unfreeze), _getActor().toBytes32())
        );
    }

    function poolManager_disallowAsset() public notGovFuzzing {
        poolManager.disallowAsset(poolId, currencyId);
    }

    // TODO: Rely / Permissions
    // Only after all system is setup
    function root_scheduleRely(address target) public notGovFuzzing {
        root.scheduleRely(target);
    }

    function root_cancelRely(address target) public notGovFuzzing {
        root.cancelRely(target);
    }

    function addToken(uint8 decimals, uint256 initialMintPerUsers) public notGovFuzzing returns (address) {
        ...
        for (uint256 i; i < actors.length; i++) {
            newToken.mint(actors[i], initialMintPerUsers);
        }

        return address(newToken);
    }

    function getMoreToken(uint8 tokenIndex, uint256 newTokenAmount) public notGovFuzzing {
        // Token Id
        ERC20 newToken = allTokens[tokenIndex % allTokens.length];

        // Mint to current actor
        newToken.mint(_getActor(), newTokenAmount);
    }
    ...
    // Step 6
    function deployTranche(uint64 poolId, bytes16 trancheId) public notGovFuzzing returns (address) {
