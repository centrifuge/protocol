...
abstract contract VaultFunctions is BaseTargetFunctions, Properties {
    ...
    // === REQUEST === //
    function vault_requestDeposit(uint256 assets, uint256 toEntropy) public updateBeforeAfter {
    ...
    function vault_requestRedeem(uint256 shares, uint256 toEntropy) public updateBeforeAfter {
    ...
    function vault_cancelDepositRequest() public updateBeforeAfter useActor {
        vault.cancelDepositRequest(REQUEST_ID, _getActor());
    }

    function vault_cancelRedeemRequest() public updateBeforeAfter useActor {
        vault.cancelRedeemRequest(REQUEST_ID, _getActor());
    }

    function vault_claimCancelDepositRequest(uint256 toEntropy) public updateBeforeAfter useActor {
        address to = _getRandomActor(toEntropy); // NOTE: We transfer to self for now
    ...
    function vault_claimCancelRedeemRequest(uint256 toEntropy) public updateBeforeAfter useActor {
        address to = _getRandomActor(toEntropy); // NOTE: We transfer to self for now
    ...
    function vault_deposit(uint256 assets) public updateBeforeAfter {
        // Bal b4
    ...
    function vault_mint(uint256 shares, uint256 toEntropy) public updateBeforeAfter {
        address to = _getRandomActor(toEntropy); // TODO: donation / changes

        // Bal b4
        uint256 trancheUserB4 = trancheToken.balanceOf(to);
        uint256 trancheEscrowB4 = trancheToken.balanceOf(address(escrow));
        ...
        // Bal after
        uint256 trancheUserAfter = trancheToken.balanceOf(to);
        uint256 trancheEscrowAfter = trancheToken.balanceOf(address(escrow));
    ...
    // TODO: Params
    function vault_redeem(uint256 shares, uint256 toEntropy) public updateBeforeAfter {
        address to = _getRandomActor(toEntropy); // TODO: donation / changes
    ...
    // TODO: Params
    function vault_withdraw(uint256 assets, uint256 toEntropy) public updateBeforeAfter {
        address to = _getRandomActor(toEntropy); // TODO: donation / changes
        ...
        // NOTE: external calls above so need to prank directly here
        vm.prank(_getActor());
        uint256 shares = vault.withdraw(assets, _getActor(), to);
