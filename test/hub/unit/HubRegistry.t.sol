...
contract HubRegistryTest is Test {
    ...
    function testPoolRegistration(address fundAdmin) public nonZero(fundAdmin) notThisContract(fundAdmin) {
        ...
        registry.registerPool(1, address(this), CENTRIFUGE_ID, USD);

        vm.expectRevert(IHubRegistry.EmptyAccount.selector);
        registry.registerPool(1, address(0), CENTRIFUGE_ID, USD);

        vm.expectRevert(IHubRegistry.EmptyCurrency.selector);
        registry.registerPool(1, address(this), CENTRIFUGE_ID, AssetId.wrap(0));

        vm.expectEmit();
        emit IHubRegistry.NewPool(newPoolId(CENTRIFUGE_ID, 1), fundAdmin, USD);
        PoolId poolId = registry.registerPool(1, fundAdmin, CENTRIFUGE_ID, USD);
    ...
    function testExists() public {
        PoolId poolId = registry.registerPool(1, makeAddr("fundManager"), CENTRIFUGE_ID, USD);
