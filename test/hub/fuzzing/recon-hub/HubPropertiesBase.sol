...
abstract contract HubPropertiesBase is Asserts, BaseBeforeAfter {
    using MathLib for D18;
    using MathLib for uint128;
    using MathLib for uint256;

    modifier stateless virtual {
        _;
        revert("stateless");
    }
    ...
    function property_total_pending_and_approved(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, PoolId[] memory createdPools) public {
    ...
    function property_total_pending_redeem_geq_sum_pending_user_redeem(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, PoolId[] memory createdPools, address[] memory actors) public {
    ...
    function property_epochId_strictly_greater_than_any_latest_pointer(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, PoolId[] memory createdPools) public {
        for (uint256 i = 0; i < createdPools.length; i++) {
            PoolId poolId = createdPools[i];
            uint32 epochId = shareClassManager.epochId(poolId);
    ...
    function property_epochId_can_increase_by_one_within_same_transaction(PoolId[] memory createdPools) public {
    ...
    function property_account_totalDebit_and_totalCredit_leq_max_int128(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, Holdings holdings, Accounting accounting, PoolId[] memory createdPools) public {
        for (uint256 i = 0; i < createdPools.length; i++) {
            PoolId poolId = createdPools[i];
            uint32 shareClassCount = shareClassManager.shareClassCount(poolId);
            // skip the first share class because it's never assigned
    ...
    function property_decrease_valuation_no_increase_in_accountValue(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, Holdings holdings, PoolId[] memory createdPools) public {
        for (uint256 i = 0; i < createdPools.length; i++) {
            PoolId poolId = createdPools[i];
            uint32 shareClassCount = shareClassManager.shareClassCount(poolId);
            // skip the first share class because it's never assigned
    ...
    function property_accounting_and_holdings_soundness(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, Holdings holdings, Accounting accounting, PoolId[] memory createdPools) public {
        for (uint256 i = 0; i < createdPools.length; i++) {
            PoolId poolId = createdPools[i];
            uint32 shareClassCount = shareClassManager.shareClassCount(poolId);
            // skip the first share class because it's never assigned
    ...
    function property_total_yield(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, Holdings holdings, Accounting accounting, PoolId[] memory createdPools) public {
        for (uint256 i = 0; i < createdPools.length; i++) {
            PoolId poolId = createdPools[i];
            uint32 shareClassCount = shareClassManager.shareClassCount(poolId);
            // skip the first share class because it's never assigned   
    ...
    function property_asset_soundness(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, Holdings holdings, Accounting accounting, PoolId[] memory createdPools) public {
        for (uint256 i = 0; i < createdPools.length; i++) {
            PoolId poolId = createdPools[i];
            uint32 shareClassCount = shareClassManager.shareClassCount(poolId);
            // skip the first share class because it's never assigned
    ...
    function property_equity_soundness(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, Holdings holdings, Accounting accounting, PoolId[] memory createdPools) public {
        for (uint256 i = 0; i < createdPools.length; i++) {
            PoolId poolId = createdPools[i];
            uint32 shareClassCount = shareClassManager.shareClassCount(poolId);
            // skip the first share class because it's never assigned
    ...
    function property_gain_soundness(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, Holdings holdings, Accounting accounting, PoolId[] memory createdPools) public {
        for (uint256 i = 0; i < createdPools.length; i++) {
            PoolId poolId = createdPools[i];
            uint32 shareClassCount = shareClassManager.shareClassCount(poolId);
            // skip the first share class because it's never assigned
    ...
    function property_loss_soundness(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, Holdings holdings, Accounting accounting, PoolId[] memory createdPools) public {
        for (uint256 i = 0; i < createdPools.length; i++) {
            PoolId poolId = createdPools[i];
            uint32 shareClassCount = shareClassManager.shareClassCount(poolId);
            // skip the first share class because it's never assigned
    ...
    function property_user_cannot_mutate_pending_redeem(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, PoolId[] memory createdPools, address[] memory _actors) public {
        for (uint256 i = 0; i < createdPools.length; i++) {
            PoolId poolId = createdPools[i];
            uint32 shareClassCount = shareClassManager.shareClassCount(poolId);
            // skip the first share class because it's never assigned
    ...
    function property_eligible_user_deposit_amount_leq_deposit_issued_amount(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, PoolId[] memory createdPools, address[] memory _actors) public stateless {
    ...
    function property_eligible_user_redemption_amount_leq_approved_asset_redemption_amount(ShareClassManagerWrapper shareClassManager, HubRegistry hubRegistry, PoolId[] memory createdPools, address[] memory _actors) public stateless {
        // loop over all created pools
        for (uint256 i = 0; i < createdPools.length; i++) {
            PoolId poolId = createdPools[i];
            uint32 shareClassCount = shareClassManager.shareClassCount(poolId);
    ...
    function property_MulUint128Rounding(D18 navPerShare, uint128 amount) public virtual {
    ...
    /// @dev Simulates the operation in the MultiShareClass::_revokeEpochShares function
    function property_MulUint128EdgeCases(D18 navPerShare, uint128 amount) public virtual {
