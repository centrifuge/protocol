...
forge verify-contract --constructor-args $(cast abi-encode "constructor(uint64,bytes16,address,uint256,address,address,address,address)" $poolId $scId $asset 0 $share $root $syncDepositManager $asyncRedeemManager) --watch --etherscan-api-key $ETHERSCAN_KEY $contract_address src/launchpad/SyncDepositVault.sol:SyncDepositVault --verifier-url $ETHERSCAN_URL --chain $CHAIN_ID
