...
forge verify-contract --constructor-args $(cast abi-encode "constructor(uint8)" $decimals) --watch --etherscan-api-key $ETHERSCAN_KEY $share src/launchpad/ShareToken.sol:CentrifugeToken --verifier-url $ETHERSCAN_URL --chain $CHAIN_ID

# forge verify-contract --constructor-args $(cast abi-encode "constructor(uint64,bytes16,address,uint256,address,address,address)" $poolId $scId $asset 0 $share $root $asyncRedeemManager) --watch --etherscan-api-key $ETHERSCAN_KEY $contract_address src/launchpad/ERC7540Vault.sol:ERC7540Vault --verifier-url $ETHERSCAN_URL --chain $CHAIN_ID
forge verify-contract --constructor-args $(cast abi-encode "constructor(uint64,bytes16,address,uint256,address,address,address,address)" $poolId $scId $asset 0 $share $root $syncDepositManager $asyncRedeemManager) --watch --etherscan-api-key $ETHERSCAN_KEY $contract_address src/launchpad/vaults/SyncDepositVault.sol:SyncDepositVault --verifier-url $ETHERSCAN_URL --chain $CHAIN_ID
